...
func TestMeterValidate(t *testing.T) {
	...
	t.Run("Water cannot be zero", func(t *testing.T) {
		fixtures := []int{
			0,
		   -5,
		   -4,
	   ...
	   for _, fixture := range fixtures {
		   ...
		   if err.Error() == "Water cannot be zero" {
			   g.Expect(err.Error()).To(Equal("Water cannot be zero"))
		   } else if err.Error() == "Water cannot be negative" {
			   g.Expect(err.Error()).To(Equal("Water cannot be negative"))
		   }
	   }
	})
